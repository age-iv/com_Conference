...
class ConferencesControllerTesis extends JController
{
...
//объ€вление переменной, определение Ѕƒ
  $db =& JFactory::getDBO();
    ...
    $context      = 'com_conference.conferencetesis.list.';
...
//фильтраци€ по поиску
    if ($search) {
	  ...
	  .'OR LOWER(name_lecture) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false )
	  .'OR LOWER(type_lecture) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false )
      .'OR LOWER(authors) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false )
	  .'OR LOWER(sections) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false )
      ;
    }
...
//запрос к таблице - выбор необходимых значений   
    $query = 'SELECT a.*, b.username, b.surname, b.name'
    . ' FROM #__tesis a'
...
//поисковый фильтр
    $lists['search']= $search;
//подключение вида    
  require_once(JPATH_COMPONENT.DS.'views'.DS.'tesis.php');
//вызов процедуры отображени€
  ConferencesViewTesis::Tesiss( $rows, $pageNav, $lists );
...
//подключение вида    
  require_once(JPATH_COMPONENT.DS.'views'.DS.'tesis.php');
//вызов процедуры отображени€
  ConferencesViewTesis::Tesis($row, $count_users, $arr_users);
  }
...
//редирект
    $this->setRedirect( 'index.php?option=com_conference&act=tesis', $msg );
  }
...
//редирект, соответственно дл€ сохранить и применить
    ...
    {
      case 'apply':
        $link = 'index.php?option=com_conference&act=tesis&task=edit&cid[]='. $post['cid'].'' ;
        break;
      case 'save':
      default:
...
// Check for request forgeries
    JRequest::checkToken() or jexit( 'Invalid Token' );
//редирект
    $this->setRedirect( 'index.php?option=com_conference&act=tesis' );
...
//сообщение: все удачно!
  ...
  function toCSV()
  {
    ...
    $query = 'SELECT a.username,a.surname,a.name,b.*'
    . ' FROM #__register a,#__tesis b'
    . ' WHERE a.username = b.username'
    . $orderby
    ;
    $db->setQuery( $query );
...
//перекодирование строк
  ...
  JFile::write(JPATH_SITE.DS.'uploads'.DS.'export'.DS.JFactory::getDate()->toFormat('%d.%m.%Y').'-tesis.csv', $fp);
  
  $link = DS.'uploads'.DS.'export'.DS.JFactory::getDate()->toFormat('%d.%m.%Y').'-tesis.csv';
